...
cls
set /p createRP="Would you like to create a system restore point? (yes/no): "
if /I "%createRP%"=="yes" (
...
echo.
color F0
echo {Credits-This File was mAde By GMbotexe}
echo [1] Clean
echo [2] Clean DNS Cache
echo [3] Disk Clean up
echo [4] Defragment and Optimize Drives
echo [5] Clear System Restore Points-(Optional don't do it if you'r not aware of what you are doing "_")
echo [6] Check Disk for Errors
echo [7] Disable Hibernation
echo [8] Clear Windows Update Cache
echo [exit] Exit
...
if "%choice%"=="1" (
    ...
    rd /s /q C:\Windows\System32\spool\printers 2>nul
    del *.log /a /s /q /f
    ...
    rd /s /q "%USERPROFILE%\AppData\Local\Mozilla\Firefox\Profiles\*.default\cache2" 2>nul

    :: Clear game-related caches (e.g., from Steam)
    echo Clearing Steam cache...
    rd /s /q "%USERPROFILE%\AppData\Local\Steam\SteamApps\downloadcache" 2>nul

    :: Clear DirectX shader cache (if applicable)
    echo Clearing DirectX shader cache...
    rd /s /q "%USERPROFILE%\AppData\Local\Microsoft\DirectX Shader Cache" 2>nul
    ...
    echo Cleaning complete. Enjoy your gaming experience!
    timeout /t 2 >nul
    call :exit_animation
    goto menu
)
...
if "%choice%"=="4" (
    color F0
    echo Opening Defragment and Optimize Drives...
    dfrgui
    timeout /t 2 >nul
    goto banner
)
...
if "%choice%"=="5" (
    ...
    echo System Restore Points cleared.
    timeout /t 2 >nul
    goto banner
)
